...
local COMMANDS = {
    ...
    lock     = function() ns.ToggleLock() end,
    grid = function(db)
    ...
    report = function()
        ...
        if channel then SendChatMessage(report, channel) else ns.Print(report) end
    end,
    macro = function()
        ns.Print("Auto:  /click WizardBuffAutoBuffButton LeftButton")
        ns.Print("Group: /click WizardBuffBrillianceButton LeftButton")
        ns.Print("Shield: /click WizardBuffShieldButton LeftButton")
    end,
}
...
function ns.HandleSlash(input)
    local cmd = ns.Trim((input or ""):lower()):match("^(%S*)") or ""

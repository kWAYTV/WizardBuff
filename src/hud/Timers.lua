...
function ns.RefreshTimerText(btn)
    ...
    local text, long
    if sec >= 3600 then text, long = string.format("%dh", math.floor(sec / 3600)), true
    elseif sec >= 60 then text, long = string.format("%dm", math.ceil(sec / 60)), true
    else text, long = string.format("%ds", math.floor(sec)), false end
    btn.timerText:SetText(text)
    btn.timerText:SetTextColor(1, long and 1 or 0.6, long and 1 or 0.3)
end
